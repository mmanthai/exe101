<template>
  ...
  <div class="p-8 flex flex-col items-center justify-between">
    <div
      class="bg-white dark:bg-gray-800 p-5 px-4 rounded-lg box-border shadow w-[45%]"
    >
        ...
        <div class="flex flex-wrap -mx-3 mb-6">
          <div class="w-full px-3">
            ...
            <div class="flex items-center pt-2">
              ...
              />
              <label
                class="ml-4 block uppercase tracking-wide text-gray-700 text-sm font-md mb-2"
                for="gr id-first-name"
              >
                Phút
              </label>
  ...
  mounted() {
    ...
    UserService.getOwnerProfile(this.userProfile.UserId)
      .then((res) => {
        ...
        if (user_profile.Vendor == null) {
          swal("Bạn chưa đăng kí cửa hàng", {
            icon: "warning",
          }).then((value) => {
            if (value) {
              location.reload();
        ...
        } else if (user_profile.Vendor.Id) {
    ...
    settingSlot() {
      ...
      if (minutes < 60 || minutes > 120) {
        ...
        if (minutes > 60 && minutes % 30 !== 0) {
          ...
          let previousSlot = {};
          let endTimeMinute_first_slot = (
            parseInt(splitOpenTime[1]) + durationMinute >= 60
              ? parseInt(splitOpenTime[1]) - durationMinute
              : parseInt(splitOpenTime[1]) + durationMinute
          ).toString();
          let endTimeHour_first_slot = (
            parseInt(splitOpenTime[1]) + durationMinute >= 60
              ? parseInt(splitOpenTime[0]) + 1 + durationHour
              : parseInt(splitOpenTime[0]) + durationHour
          ).toString();
          let first_slot = {
            No: 1,
            SlotNo: 1,
            StartTime: `${splitOpenTime[0]}:${splitOpenTime[1]}`,
            EndTime: `${endTimeHour_first_slot.padStart(
              2,
              "0"
            )}:${endTimeMinute_first_slot.padStart(2, "0")}`,
            Price: 0,
          ...
          for (let i = 2; i <= totalSlot; i++) {
            ...
            let startTimeMinutes =
              parseInt(previousEndTime[1]) + durationMinute >= 60
                ? (parseInt(previousEndTime[1]) - durationMinute).toString()
                : `${parseInt(previousEndTime[1]) + durationMinute}`;
            let startTimeHours =
              parseInt(previousEndTime[1]) + durationMinute >= 60
                ? parseInt(previousEndTime[0]) + 1
                : parseInt(previousEndTime[0]);
            ...
            if (hourEndTime > parseInt(splitCloseTime[0])) {
              ...
              hourEndTime == parseInt(splitCloseTime[0]) &&
              minuteEndTime > parseInt(splitCloseTime[1])
            ) {
              minuteEndTime = parseInt(splitCloseTime[1])
                .toString()
                .padStart(2, "0");
            ...
            let slot = {
              No: i,
              SlotNo: i,
              StartTime: `${previousEndTime[0].padStart(
                2,
                "0"
              )}:${previousEndTime[1].padStart(2, "0")}`,
            ...
            if (this.slot_array.length > 0) {
              ...
              sessionStorage.setItem(
                "slot_array",
                JSON.stringify(this.slot_array)
              );

              swal({
                title:
                  "Cài đặt thành công. Xin vui lòng cài đặt giá tiền cho từng loại sân !",
                icon: "success",
              }).then(() => {
