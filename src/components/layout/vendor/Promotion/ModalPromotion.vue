      ...
      <div class="relative bg-white rounded-lg shadow dark:bg-gray-700 mt-10">
        ...
        <div class="py-6 px-6 lg:px-8">
          ...
          <form @submit.prevent="updatePromotion">
            <div>
              <div
                class="photo-wrapper p-2 flex flex-col items-center justify-center"
              >
                <img
                  @load="closeWaiting"
...
export default {
  ...
  methods: {
    ...
    changeFormatDatePicker(EffectiveDate, ExpiredDate) {
      let effectiveDate_string = new Date(
        new Date(ExpiredDate).getTime() -
          new Date(ExpiredDate).getTimezoneOffset() * 60000
      )
        .toISOString()
        .split("T")[0];

      let expiredDate_string = new Date(
        new Date(ExpiredDate).getTime() -
          new Date(ExpiredDate).getTimezoneOffset() * 60000
      )
        .toISOString()
  ...
  watch: {
    detail() {
      ...
      this.changeFormatDatePicker(
        this.promotion.EffectiveDate,
        this.promotion.ExpiredDate
      );
      this.isClose = false;
    },
    click() {
