            ...
            <div class="image overflow-hidden">
              <img
                ...
                :src="
                  promotion.ImageUrl
                    ? promotion.ImageUrl
                    : 'https://i.ibb.co/t41GMzQ/discount.png'
                "
                alt="ảnh khuyến mãi"
              />
            </div>
        ...
        <div class="w-full md:w-3/4 mx-2 h-full">
          ...
          <div class="bg-white p-3 shadow-sm rounded-sm">
            ...
            >
              ...
              <div>
                <label class="text-[#747474]" for="username">% giảm giá</label>
                <div class="relative">
                  <p
                    v-if="!promotion.DiscountPercentage"
                    class="text-2xl text-red-500 absolute right-12 top-4"
                  >
                    *
                  </p>
                  <input
                    ...
                    min="1"
                    v-model.trim="promotion.DiscountPercentage"
                  />
                  <!-- <p v-if="err.errVendorName" class="
                        absolute
                        top-[138%]
  ...
  methods: {
    ...
    createPromotion() {
      ...
      UserService.getOwnerProfile(userId)
        .then((res) => {
          console.log(res);
          this.promotion.VendorId = res.data.VendorId;
          if (this.promotion.VendorId !== 0) {
            console.log(this.promotion);
            if (
              this.promotion.DiscountPercentage < 1 ||
              this.promotion.DiscountPercentage > 81
            ) {
              swal("Xin vui lòng nhập giảm giá từ 1 đến 80%", {
                icon: "warning",
              }).then(() => {
                return;
              });
