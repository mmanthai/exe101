<template>
  ...
  <div class="flex flex-col items-center justify-between">
    <div
      class="mt-2 bg-white dark:bg-gray-800 p-5 w-full px-4 rounded-md box-border"
    >
        ...
        <div class="container mx-auto flex px-4">
          <div class="mx-auto flex items-center justify-center">
            ...
            <div class="flex items-center relative mt-6">
              <span
                @click="searchPromotion(1)"
                class="text-2xl p-2 text-gray-400 absolute right-0"
              >
                <Icon icon="ei:search" />
              </span>
              <input
                @keyup.enter="searchPromotion(1)"
                v-model="param.query"
                type="text"
        ...
        >
          ...
          <tbody>
            ...
            >
              <td class="px-6 py-4">
                <div class="flex items-center">
                  <div class="flex-shrink object-contain">
                    <img
                      @load="closeWaiting"
                      class="rounded-2xl mr-3 w-10 h-10 object-contain"
                      :src="items.ImageUrl"
                    />
                  </div>
              ...
              <td class="px-6 py-4">
                {{ new Date(items.EffectiveDate).toLocaleDateString() }} -
                {{ new Date(items.ExpiredDate).toLocaleDateString() }}
              </td>
              <td class="px-6 py-4">
                <div class="mb-3 mt-2">
                  <p
                    class="text-normal text-green-600 font-semibold"
                    v-if="items.status === 1"
                  >
                    Áp Dụng
                  </p>
              ...
              <td class="px-6 py-4">
                <span class="flex items-center">
                  <Icon
                    ...
                    class="w-6 h-6 cursor-pointer hover:text-gray-600 duration-200"
                    icon="ant-design:eye-filled"
                  />
                  <Icon
                    @click="deletePromotion(items.Id)"
                    class="w-6 h-6 cursor-pointer hover:text-gray-600 duration-200"
                    icon="bxs:trash-alt"
                  />
                </span>
  ...
  <!--The Modal-->
  <ModalPromotion
    @loadAgain="promotionPageList(this.param.currentPage)"
...
export default {
  ...
  created() {
    const userId = TokenService.getUser().Token.UserId;
    UserService.getOwnerProfile(userId)
      .then((res) => {
        this.param.vendorId = res.data.VendorId;
  ...
  methods: {
    ...
    //   UserService.getOwnerProfile(userId)
    //     .then((res) => {
    //       return (this.promotion.VendorId = res.data.VendorId);
    //     })
    //     .catch((err) => {
    //       console.log(err);
    //       this.closeWaiting();
    ...
    promotionPageList(page) {
      ...
      if (this.param.currentPage > this.displayInfo.pageCount) {
        ...
        UserService.getOwnerProfile(userId)
          .then((res) => {
            this.param.vendorId = res.data.VendorId;
            console.log(this.param.vendorId + "-" + res.data.VendorId);
