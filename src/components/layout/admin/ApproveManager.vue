<template>
            ...
            <div class="mx-auto flex items-center justify-center">
              ...
              <div class="flex items-center relative mt-6">
                ...
                <input type="text" placeholder="Search..."
                  class="pr-10 rounded border border-gray-500 bg-gray-50 placeholder:text-sm focus:ring-600" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="w-[100%] h-full">
            <div class="container mx-auto h-96 mt-8 md:mt-0 min-w-full">
              <table class="min-w-full">
                <th
                  class="px-6 py-3 text-sm font-medium leading-4 tracking-wider text-left text-[#334D6E] bg-gray-100 border-b border-gray-200 cursor-pointer">
                  Họ Tên
                ...
                <tbody class="bg-white">
                  <tr v-for="member in approveRequestInfo" :key="member.id">
                    ...
                    <td class="px-6 py-4 border-b border-gray-200 whitespace-nowrap">
                      <div class="flex items-center">
                        <span v-if="member.Vendor.ApproveStatusId == 2" class="font-semibold text-pink-500">Đang Chờ
                          Duyệt</span>
                        <span v-else-if="member.Vendor.ApproveStatusId == 3" class="font-semibold text-[#50D222]">Đã
                          Duyệt</span>
                        <span v-else-if="member.Vendor.ApproveStatusId == 4" class="font-semibold text-red-500">Từ
                          Chối</span>
                      </div>
                    </td>
                    <td class="px-2 py-4 border-b border-gray-200 whitespace-nowrap">
                      <div v-if="member.Vendor.ApproveStatusId == 2" class="flex items-center">
                        <div class="ml-4 text-[#334D6E]">
                          <div class="flex items-center">

                            <Icon @click="showDetail(member.id)"
                              class="w-8 h-8 mr-2 cursor-pointer text-gray-500 hover:text-gray-700 active:text-gray-800 duration-200"
                              icon="ic:sharp-remove-red-eye" />

                            <Icon @click="AcceptRequest(member.Vendor.Id)"
                              class="w-7 h-7 mr-2 cursor-pointer text-green-500 hover:text-green-700 active:text-green-800 duration-200"
                      ...
                      <div v-else class="flex items-center">
                        <div class="ml-4 text-[#334D6E]">
                          <div class="flex items-center">
                            <Icon @click="showDetail(member.Id)"
                              class="w-7 h-7 mr-2 cursor-pointer text-gray-500 hover:text-gray-700 active:text-gray-800 duration-200"
              ...
              <nav v-show="pageCount > 1" aria-label="Page navigation example mx-auto">
                <ul class="inline-flex -space-x-px">
                  ...
                  <li v-for="page in 2" :key="page">
                    <span v-if="page === 1" aria-current="page"
  ...
  <!--The Modal-->
  <ModalOwnerDetail @accept="AcceptRequest" @decline="showAlert" :button="
  ...
  data() {
    return {
      ...
      param: {
        ...
        query: "",
        currentPage: 1,
        statusId: "", // statusId : 1 , 2 3 ,4 (reject)
      },
      displayInfo: {
        pageCount: 0,
        totalRequest: 0,
            ...
            UserService.getOwnerProfile(info.InsertedBy)
              .then((res) => {
                if (res.data) {
                  ...
                  console.log(object_request.Vendor)
